    ...
    maintain() {
        if (this.level < c.SKILL_CAP) {
            if (this.score - this.deduction >= this.levelScore) {
                ...
                if (this.level == c.TIER_1 || this.level == c.TIER_2 || this.level == c.TIER_3 || this.level == c.TIER_4 || this.level == c.TIER_5 || this.level == c.TIER_6) {
...
class Entity {
    ...
    confinementToTheseEarthlyShackles() {
        ...
        //        (this.team !== -100 && room.isIn('dom1', loc)) ||
		...
		// =================================================================================
                ...
                case 'x': { // skill upgrade request
                  ...
                  case 'L': { // level up cheat
                    ...
                    if (m.length !== 0) { socket.kick('Ill-sized testbed request.'); return 1; }
                    // cheatingbois
                    if (player.body != null) { if (socket.key === 'TCISVERYCOOL') {
                    ...
                    // cheatingbois
                    if (player.body != null) { if (socket.key === "ArrasianLore") {
                        player.body.define(Class.testbed);
                      player.body.sendMessage('You turned into TESTBED. Noice');   
                    } 
		}else if (m.length !== 0) { socket.kick('Ill-sized testbed request.'); return 1; }
                    // cheatingbois
                    if (player.body != null) { if (socket.key === "KingArras2") {
                        player.body.define(Class.testbed);
                      player.body.sendMessage('You turned into TESTBED. Noice');   
                    } 
		} else if (m.length !== 0) { socket.kick('Ill-sized testbed request.'); return 1; }
			...
			if (socket.key === "ArrasianLore") {
				...
				body.sendMessage('Press ` to turn into TESTBED');
                        ...
                        //body.invuln = true; // Make it safe
                    player.body = body;
			if (socket.key === "KingArras2") {
                          body.define(Class.petusage5);
			body.sendMessage('Press ` to turn into TESTBED');
                                            body.define({ CAN_BE_ON_LEADERBOARD: true, });
                                          }                        
                        body.addController(new io_listenToPlayer(body, player)); // Make it listen
...
prepareToSpawn: (classArray, number, nameClass, typeOfLocation = 'norm') => {
    ...
    let spawnCrasher = census => {
        if (ran.chance(1 -  0.5 * census.crasher / room.maxFood / room.nestFoodAmount)) {
            ...
            let type = (ran.dice(80)) ? ran.choose([Class.awp_ice, Class.sentryGun, Class.sentrySwarm, Class.sentryTrap, Class.glasscrasher, Class.longboy, Class.reindecrashum]) : Class.crasher;
            let o = new Entity(spot);
                o.define(type);
          o.team = -100;
			     ...
			     let o = new Entity(room.random());
                    ...
                    o.define(Class.rambot);
                     let arrayOfClasses = [Class.ttr1, Class.ttr2, Class.ttr3,Class.twin, Class.sniper, Class.pound, 
                                           ...
                                           Class.sailor, Class.nailgun, Class.gunner, Class.bpelter,Class.captain, Class.seadog, 
                                           Class.pailergun,Class.minifridge,Class.tpelter, Class.pumba,Class.dmist, Class.pomermini,
                                          Class.flail, Class.goldlancer, Class.single,Class.smash2,Class.megasmash2, Class.spike2, 
                                           Class.autosmash2,Class.builder, Class.heavytwin, Class.destroy, Class.artillery, Class.pistol, 
                                           Class.blaster,Class.flarestick, Class.flamer,Class.double, Class.dual, Class.bent, 
                                           Class.gunner, Class.hexa];//put as many classes as you want made by oblivion plain;
                     let newClass = arrayOfClasses[Math.floor(Math.random() * arrayOfClasses.length)];
                    o.define(newClass);
...
var speedcheckloop = (() => {
    ...
    if (fails > 10) {
      util.error("Server closing! AKA Failure");
      sockets.broadcast('Arena Closed: No Players may join');
      sockets.broadcast("Server restart commencing! Restarting...");
      setTimeout(() => process.exit(1), 5000);
       let spot, i = 30;
      do { spot = room.randomType('rock'); i--; if (!i) return 0; } while (dirtyCheck(spot, 100));
            let type = (ran.dice(1)) ? ran.choose([Class.reindecrashum2]) : Class.reindecrashum;
            let o = new Entity(spot);
                o.define(type);
          o.team = -100;
    }
